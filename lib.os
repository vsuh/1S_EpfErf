...
Процедура Шаг_unpack(Скрипт, Отказ) Экспорт
	...
	Для Каждого фл Из мсФайлы Цикл
		СтрокаКоманды = "v8\v8unpack.exe -p [##] [@@]";
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "[##]", """"+фл.ПолноеИмя+"""");
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "[@@]", """"+РабочийКаталог+"\"+фл.Имя+"""");
...
Процедура Шаг_pack(Скрипт, Отказ) Экспорт
	...
	КодВозврата = 0;
	Для Каждого фл Из мсФайлы Цикл
		СтрокаКоманды = "v8\v8unpack.exe -b [##] [@@]";
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "[##]", """"+фл.ПолноеИмя+"""");
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "[@@]", """"+ВыхКаталог+"\"+фл.Имя+"""");
		ЗапуститьПриложение(СтрокаКоманды, , Истина, КодВозврата);
